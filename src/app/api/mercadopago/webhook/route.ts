...
const STATUS_MAP: Record<string, string> = { authorized: 'active', pending: 'incomplete', paused: 'past_due', cancelled: 'canceled', canceled: 'canceled' }
...
export async function POST(request: NextRequest) {
  ...
  if (secret && signature) {
    try {
      WebhookSignatureValidator.validate({ xSignature: signature, xRequestId: request.headers.get('x-request-id') ?? '', dataId, secret })
    } catch {
      return NextResponse.json({ error: 'Invalid signature' }, { status: 401 })
    }
  }

  if (!signature && dataId === '123456') return NextResponse.json({ received: true, test: true })
  if (body.type && body.type !== 'subscription_preapproval') return NextResponse.json({ received: true })

  // Preview procesa eventos Sandbox; Production procesa eventos reales.
  const token = process.env.VERCEL_ENV === 'preview' && process.env.MERCADOPAGO_TEST_ACCESS_TOKEN
  ...
  const { data: existing } = await admin.from('subscriptions').select('id').eq('mercadopago_subscription_id', subscription.id).maybeSingle()
  const row = {
    organization_id: reference.organizationId, plan_id: reference.planId, status,
    ...
    canceled_at: canceledAt, updated_at: new Date().toISOString(),
  }
  ...
  await admin.from('organizations').update({ subscription_plan: status === 'active' ? reference.tier : 'basic' }).eq('id', reference.organizationId)
  if (status === 'active') {
    await admin.from('brands').update({ status: 'approved' }).eq('organization_id', reference.organizationId).eq('status', 'suspended')
  }
  return NextResponse.json({ received: true })
}
