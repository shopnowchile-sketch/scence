...
type UploadedDocument = { id: string; document_type: string; title: string; original_filename: string; mime_type: string; file_size: number; visibility: 'private' | 'approved_brands'; created_at: string }
  ...
  return <div className="mx-auto max-w-3xl space-y-5">
    <section className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
      ...
      <form onSubmit={uploadDocument} className="mt-4 grid gap-3 sm:grid-cols-[150px_1fr]">
        <select name="document_type" required className="rounded-lg border border-gray-200 px-3 py-2 text-sm"><option value="identity">Cédula de identidad</option><option value="portfolio">Portfolio</option><option value="other">Otro</option></select>
        <input name="title" required maxLength={120} placeholder="Ej: Cédula de identidad" className="rounded-lg border border-gray-200 px-3 py-2 text-sm" />
        <div className="sm:col-span-2 grid gap-2 sm:grid-cols-2"><div><label className="mb-1 block text-xs font-semibold text-gray-500">Archivo</label><input name="file" required type="file" accept=".pdf,.jpg,.jpeg,.png,.webp" className="w-full rounded-lg border border-dashed border-gray-300 p-3 text-sm text-gray-600 file:mr-3 file:rounded-lg file:border-0 file:bg-violet-50 file:px-3 file:py-2 file:font-semibold file:text-violet-700" /></div><div><label className="mb-1 block text-xs font-semibold text-gray-500">Compartir</label><select name="visibility" defaultValue="private" className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm"><option value="private">Privado · solo yo</option><option value="approved_brands">Compartir con marcas aprobadas</option></select></div></div>
