-- Validate and normalize Instagram handles during influencer self-registration.
-- No schema changes: hardens the existing handle_new_user() trigger.
...
begin
  if v_is_self_registration then
    ...
    -- Accept @usuario, usuario or an Instagram URL and persist the normalized handle.
    ...
    v_instagram_username := lower(trim(v_instagram_username));

    if nullif(v_instagram_username, '') is null
      or v_instagram_username ~ '@'
      or v_instagram_username !~ '^[a-z0-9._]{1,30}$'
    then
  ...
  if v_role = 'influencer'::public.user_role then
    ...
    if v_influencer_id is not null then
      ...
      insert into public.influencers (
        user_id, email, display_name, commune, address, birth_date,
      ...
      values (
        ...
        v_commune, v_address, v_birth_date, true, v_scence_org_id,
