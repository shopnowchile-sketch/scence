...
const VISIBILITIES = new Set(['private', 'approved_brands'])
...
export async function GET() {
  ...
  const { data, error } = await ctx.admin.from('influencer_documents').select('id, document_type, title, original_filename, mime_type, file_size, visibility, created_at').eq('influencer_id', ctx.influencer.id).order('created_at', { ascending: false })
...
export async function POST(request: NextRequest) {
  ...
  const file = form.get('file'), documentType = String(form.get('document_type') ?? ''), title = String(form.get('title') ?? '').trim(), visibility = String(form.get('visibility') ?? 'private')
  ...
  if (!VISIBILITIES.has(visibility)) return NextResponse.json({ error: 'Visibilidad inválida.' }, { status: 422 })
