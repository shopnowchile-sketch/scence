...
| 2.3 (permisos admin sobre campañas de marca) | Pri reportó "no aparecen las campañas creadas por marcas" en `/admin-campaigns`. Diagnóstico con Supabase MCP: cada marca auto-registrada queda con su propia `organization_id` aislada (creada por el trigger de signup), distinta a la de "Scence SpA" — `GET /api/campaigns` filtraba estrictamente por `organization_id` del usuario, así que admin nunca veía campañas de marcas con org propia. Fix acotado: admin/`super_admin`/owner ve todas las campañas sin filtro de org; Marca sigue viendo solo las suyas (endpoint propio sin cambios). Reportado como B-22 abajo — se expandió a 7 archivos más en la misma sesión (ver siguiente fila) | 2026-07-03 |
| 2.3 (mismo bug en rutas hermanas) | A pedido explícito de Pri ("revisa todas las rutas donde se podría generar este mismo error"), se auditaron ~46 rutas que tocan `organization_id`. El mismo patrón (admin bloqueado o con alcance reducido en campañas de marcas con org propia) apareció en 7 archivos más, todos corregidos con el mismo criterio (`getUserRole` → bypass de filtro solo para admin): `campaigns/[id]/route.ts` (detalle, editar, activar/pausar/completar, eliminar — antes daba 404 apenas se abría desde la lista), `campaigns/[id]/applications/route.ts` (aceptar/rechazar postulaciones), `campaigns/[id]/brands/route.ts` (agregar/quitar marcas colaboradoras), `campaigns/[id]/assets/route.ts` + `[assetId]/route.ts` (ver/subir/borrar archivos — además se corrigió que el asset quedaba tageado con la org del admin en vez de la org real de la campaña), `campaigns/[id]/influencer-report/route.ts` (reporte PDF), `campaigns/[id]/notify-influencers/route.ts` (el botón manual de B-continuación abajo limitaba el alcance a la org, no bloqueaba). No se tocó `invoices/route.ts` (mismo filtro, pero es facturación — fuera de alcance sin aprobación explícita aparte) | 2026-07-03 |
...
| — | 🔴 Crítica | Admin/Marca | `/api/influencers/ranking` y `/api/brand/influencers/ranking` traían TODOS los influencers de la org sin límite en la query, ordenaban, y recién ahí cortaban la respuesta a un `limit` (máx 500, default 200). La organización real tiene 1452 influencers — con `limit=500` (AddInfluencerClient, "Agregar influencer") o `limit=150` (las 2 páginas de Ranking) la mayoría del roster nunca llegaba al frontend, sin ningún error — Pri reportó "hay influencers que no me aparecen" al agregar a una campaña | ✅ Corregido — cap subido de 500 a 5000 en ambos endpoints (sin costo extra de query, ya se traía todo antes de cortar); los 3 callers piden `limit=5000`. **Nota de escala:** con 1452+ filas estas vistas siguen sin paginar ni virtualizar (una sola tabla client-side) — aceptable hoy, pero si el roster crece mucho más va a necesitar paginación real server-side |
| — | 🔴 Crítica | Admin | "Live influencers" en el dashboard admin siempre mostraba 0. Dos causas: (1) `isRecentlyOnline()` buscaba `last_seen_at`/`last_activity_at`/etc. pero `/api/influencers` enriquece cada fila con ese dato bajo el nombre `last_sign_in_at` — nunca hacía match; (2) ninguna parte del código escribía `profiles.last_seen_at` — ni trigger ni endpoint, columna huérfana | ✅ Corregido — `isRecentlyOnline` ahora también chequea `last_sign_in_at`; nuevo `POST /api/presence/heartbeat` (actualiza `profiles.last_seen_at=now()`) llamado por `<PresenceHeartbeat />` cada 2 min mientras el portal influencer está abierto (umbral de "en línea" es 10 min) |
| — | 🟢 Feature | Admin | Pri pidió sacar "Finanzas este mes" del dashboard admin | ✅ Movido a `/admin-billing` (reusa `kpis.revenue_month/payroll_month/margin_pct` de `/api/dashboard`, sin lógica nueva) |
| — | 🟢 Feature | Admin | Live influencers no era clickeable; no había forma de invitar/reenviar invitación a influencers sin acceso desde la lista (solo desde el perfil, uno por uno) | ✅ Live influencers ahora linkea a `/admin-influencers/[id]`. Botón "Invitar" nuevo en cada fila de la lista (si no tiene acceso), reutiliza `POST /api/influencers/[id]/invite` ya existente. Header "Estado" ahora ordenable (`is_active`) |
| B-21 | 🔴 Crítica | Admin/Marca | UAT en vivo (pedido por Pri) encontró: (1) dashboard mostraba "1000" influencers en vez de 1452 — regresión del fix de B-20, el fetch nuevo no tenía `.limit()`; (2) mismo bug en Ranking Admin/Marca — "Mostrando 1000 influencers", el fix previo del cap (500→5000) subió solo el límite de respuesta, no el de la query real, y PostgREST corta en 1000 filas por default | ✅ Corregido — `.limit()` explícito agregado a todas las queries afectadas (dashboard, ranking Admin, ranking Marca). Resto del UAT (filtros/sorts de Influencers, Campañas, Marcas) verificado OK sin cambios |
| B-20 | 🔴 Crítica | Admin | Con el fix anterior ya desplegado, Pri reportó que "Live influencers" seguía sin mostrar a quién está conectada, y que "Acceso al portal de influencers" mostraba números claramente rotos ("35 han ingresado / 64 aún no ingresan" — 99 en total, no 1452). Causa real: ambas métricas se calculaban en el cliente (`DashboardClient.tsx`) filtrando el array de `fetch('/api/influencers?limit=100')` — con 1452 influencers reales, esa lista capada nunca representaba al roster completo, y la influencer que sí tenía heartbeat activo no caía dentro de esas primeras 100 filas | ✅ Corregido — `/api/dashboard` ahora calcula `influencer_portal.{entered,pending}` y `live_influencers` server-side, trayendo `id, user_id, display_name` de TODA la org (sin límite de fila, columnas mínimas) y cruzando contra `profiles.last_seen_at`, en vez de contar sobre una lista ya recortada. Verificado con SQL contra la org real: total 1452, entered 78 (antes "35" de un subconjunto de 99). `DashboardClient.tsx` usa sentinel `-1` (no `\|\| fallback`) al leer estos 2 campos porque 0 es un valor real válido — mismo patrón de bug que los KPIs financieros (ver fila anterior) |
| — | 🔴 Crítica | Admin | Los 3 KPIs financieros del dashboard ("Facturado", "Costos recibidos", "Margen bruto") daban `$0`/`0%` siempre — buscaban nombres de campo (`revenue`, `facturado`, `payroll`, `inboundCosts`) que no existen en la respuesta real de `/api/dashboard` (`kpis.revenue_month`, `kpis.payroll_month`, `kpis.margin`, `kpis.margin_pct`). Además "Costos recibidos" describía un concepto inexistente en el schema (`invoices` no tiene columna `direction`; no hay "facturas entrantes" de una marca — eso ya es "Facturado"), duplicando esa tarjeta en vez de mostrar el costo real. "Influencers en roster" decía "activos" pero el conteo no filtra por `is_active` | ✅ Corregido — keys reales agregadas; tarjeta repropuesta como "Payroll pagado" (costo real: pago a influencers); subtítulos corregidos |
| — | 🟢 Feature | Influencer | `/inf-dash` era un "kitchen sink": KPIs, campañas disponibles para postular, gráfico de avance, tareas pendientes, campañas activas con submit inline, eventos/bookings y pagos, todo en un solo scroll largo — pedido de Pri: la pantalla principal debe mostrar solo campañas con su estado | ✅ Reconstruido — ahora solo conteo rápido (activas/postuladas/completadas) + lista "Mis campañas" con badge de estado (usa `application_status` además de `campaign.status`, antes no se leía). El resto se reubicó en tabs ya existentes, sin duplicar: postular a campañas abiertas → `/inf-campaigns` (sección nueva reusando `/api/influencer/campaigns/open`, ya existía); eventos de campaña (tabla `events`, antes sin tab propio) → `/inf-bookings` (sección nueva); entregables y pagos ya vivían en `/inf-tasks` sin cambios. De paso, `/api/influencer/campaigns` ahora expone `application_status` (campo aditivo, no afecta a `inf-profile`/`inf-tasks`, los otros 2 consumidores) |
| — | 🟢 Feature | Admin | Rediseño del dashboard admin: Pri reportó "la interfaz está fea" sobre las tarjetas KPI y el bloque Live influencers/adopción de portal. Se preguntó alcance explícito vía pregunta directa; Pri acotó a 2 puntos (sin overhaul visual general, sin mockups previos) | ✅ Construido — (1) las 6 tarjetas KPI se agrupan en 2 secciones con etiqueta ("Operación": campañas/influencers/marcas; "Finanzas este mes": facturado/payroll/margen, en panel con fondo tenue para diferenciarlo); (2) "Live influencers" dejó de ser una sección completa aparte (con mucho espacio vacío cuando da 0) y pasa a la misma fila de 3 columnas que las 2 tarjetas de adopción de portal, con empty-state más compacto (ícono + texto corto) |

| B-22 | 🔴 Crítica | Admin | Admin no veía campañas creadas por marcas (auto-registradas quedan con `organization_id` propia y aislada, distinta a "Scence SpA"); al expandir la auditoría (pedida por Pri) apareció el mismo bloqueo en 7 archivos más — ver changelog "2.3 (permisos admin sobre campañas de marca)" y "2.3 (mismo bug en rutas hermanas)" para el detalle completo por archivo | ✅ Corregido — `getUserRole` bypassa el filtro de `organization_id` solo para admin/`super_admin`/owner, en los 8 archivos afectados (`GET /api/campaigns` + 7 rutas de detalle/acciones de campaña). Marca sin cambios (endpoints propios) |
| B-23 | 🔴 Alta | Admin | Wizard "Nueva campaña" (`CampaignForm.tsx`, Paso 3) tenía un toggle duplicado de visibilidad con valores inválidos (`'public'`/`'invite_only'` en vez de `'open'`/`'private'`) — mismo defecto que B-16 pero en el formulario de creación, no en `CampaignDetail.tsx`. Al fallar la validación sin manejador de error, "Crear campaña" no hacía nada sin ningún aviso | ✅ Corregido — toggle duplicado eliminado (Paso 1 ya tenía el control correcto); se agregó manejador de validación que muestra paso + campo exacto. Reportado urgente por Pri |
